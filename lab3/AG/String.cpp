...
String& String::erase(size_t pos, size_t len) {
    ...
    size_t firstErase = pos;
    size_t noErase = pos+len;
    ...
    if(len == npos || pos + len > sz){
        ...
        while(cstr[noErase] != a_null_byte){
            cstr[firstErase] = cstr[noErase];
            firstErase++;
            noErase++;
        }

        cstr[sz-len] = a_null_byte;
        sz = firstErase;
    }
...
String& String::insert(size_t pos, const String& str) {
    ...
    size_t endIndex = sz-1;
    String temp = str;

    // shifting
    for(size_t i = 0; i < sz-pos; i++){
        cstr[endIndex+str.sz] = cstr[endIndex];
        endIndex--;
    }
